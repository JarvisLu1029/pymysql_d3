function renderPieChart(data, selector = "#pie_chart") {
    const pie_width = 500;
    ...
    svg.append("g")
        ...
        .attr("d", arc)
        .on("click", (event, d) => {
            fetch(`/api/sub_category_pie/${d.data.label}`)
                .then(response => response.json())
                .then(data => {
                    renderPieChartSubCategory(data);
...
function renderPieChartSubCategory(data, selector = "#pie_chart") {
    const pie_width = 500;
    const pie_height = 500;
    const radius = Math.min(pie_width, pie_height) / 2;
