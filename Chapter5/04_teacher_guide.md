...
| 3 | `AND`、`OR`、`BETWEEN`、布林 | 依 `is_humanoid` 套用不同足跡區間 |
...
- 2026 年 8 月 20 日晚上 18:00 至 22:00，早川秋、洛伊德、蕾潔、蝙蝠惡魔、帕瓦、波奇塔、彭德與淀治都曾成功進入 R03。前四人在 21:00 前已離開；縮到案發區間 21:10 至 21:25 後，優先調查對象才剩帕瓦、波奇塔、彭德與淀治。帕瓦在金庫前玩按鈕尤其可疑，但不是最終犯人。
- 鑑識設定的人型鞋印區間為 23.0～28.0，非人型足跡區間為 10.0～28.0。搭配 `has_orange_trace = TRUE` 後，會留下人型的淀治與帕瓦，以及非人型的波奇塔與彭德；四者都可能留下橘色痕跡，因此第三關仍不能定案。
...
-- 第三關：依人型／非人型套用不同足跡區間，再比對橘色痕跡
SELECT name, shoe_size, is_humanoid, has_orange_trace
FROM suspects
WHERE has_orange_trace = TRUE
  AND (
       (is_humanoid = TRUE AND shoe_size BETWEEN 23.0 AND 28.0)
       OR
       (is_humanoid = FALSE AND shoe_size BETWEEN 10.0 AND 28.0)
  )
ORDER BY suspect_id;
