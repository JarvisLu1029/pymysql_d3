...
CHAPTERS = (
    Chapter(1, "案件卷宗", "午夜失竊", "SELECT / View", "失竊案發生在哪一個房間編號？", "例如：R00", "load_case_brief", (), "預期欄位：case_name、estimated_start、discovered_at、crime_room、mission", ("r03", "Ｒ０３"), lambda rows: _contains(rows, "午夜藍寶石失竊案", "R03")),
    Chapter(2, "門禁紀錄", "不在場證明的裂縫", "WHERE / BETWEEN", "案發區間成功進入 R03 的人員編號是？", "輸入人員編號", "find_successful_r03_entries", (CASE_START, "2026-08-20 21:25:00"), "預期欄位：suspect_id、access_time；只可包含 SUCCESS + IN", ("102",), lambda rows: len(rows) == 1 and _contains(rows, "102")),
    Chapter(3, "現場物證", "鞋印與深藍纖維", "AND / 條件組合", "同時符合 27.5 號鞋印與藍色外套的兩人是？", "以逗號分隔兩個姓名", "find_physical_matches", (27.5,), "預期欄位：name、shoe_size、has_blue_jacket", ("林冠宇,趙偉倫", "趙偉倫,林冠宇"), lambda rows: len(rows) == 2 and _contains(rows, "林冠宇", "趙偉倫", "27.5")),
    Chapter(4, "影像與消費", "銀色容器", "多表 JOIN", "監視器攜帶物與消費品項共同指向什麼物件？", "輸入物件名稱", "find_camera_and_purchase_clues", (CASE_START, CASE_END), "預期欄位：name、seen_time、location、carrying、item_name", ("銀色保溫袋", "保溫袋"), lambda rows: bool(rows) and _contains(rows, "銀色保溫袋", "林冠宇")),
    Chapter(5, "數位鑑識", "被刪除的請求", "LIKE / 自我 JOIN", "是誰要求在 21:10 左右進行斷電測試？", "輸入姓名", "find_deleted_messages", ("斷電",), "預期欄位：sender_name、receiver_name、sent_at、message_text、is_deleted", ("林冠宇",), lambda rows: bool(rows) and _contains(rows, "林冠宇", "張博翔", "斷電")),
    Chapter(6, "最終逮捕令", "五條證據鏈", "DISTINCT / 多表 JOIN", "所有數位證據唯一指向誰？", "輸入完整姓名", "find_prime_suspect", (CASE_START, CASE_END), "預期欄位：suspect_id、name、job_title，且只能有一列", ("林冠宇", "102"), lambda rows: len(rows) == 1 and _contains(rows, "102", "林冠宇", "保全主管")),
)
...
def create_app() -> FastAPI:
    app = FastAPI(title="午夜藍寶石失竊案", docs_url=None, redoc_url=None)
    ...
    async def index(request: Request):
        ...
        chapter = CHAPTERS[unlocked] if unlocked < len(CHAPTERS) else None
        ...
        return templates.TemplateResponse(request=request, name="index.html", context={
            ...
            "query_error": query_error, "total": len(CHAPTERS),
        })
    ...
    async def check_answer(request: Request, payload: AnswerPayload):
        ...
        return {"ok": True, "solved": solved, "message": "證據吻合。逮捕令已完成。" if solved else "證據吻合，下一份卷宗已解鎖。", "next_url": "/#active-case"}
